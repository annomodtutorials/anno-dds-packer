...
# Matches one or more trailing numeric index segments like _0, _1, _001, _0_1
_TRAILING_INDEX_RE = re.compile(r"([._\-]+\d+)+$")


def _suffix_match(stem: str) -> tuple[str, MapType | None]:
    """Core per-stem suffix scan (no index stripping). Returns (base, mt) or (stem, None)."""
    low = stem.lower()
...
def detect_suffix(stem: str) -> tuple[str, MapType | None]:
    ...
    Also handles trailing numeric indices after the suffix, e.g.
    ``wall_height_0`` is detected as HEIGHT with base ``wall_0`` so that
    ``wall_diff_0`` / ``wall_height_0`` / ``wall_normal_0`` all group under
    the same set key ``wall_0``.
    """
    # First: direct match
    base, mt = _suffix_match(stem)
    if mt is not None:
        return base, mt

    # Second: strip trailing numeric index segments and retry
    m = _TRAILING_INDEX_RE.search(stem)
    if m:
        core = stem[: m.start()]          # e.g. "wall_height" from "wall_height_0"
        index_tail = stem[m.start():]     # e.g. "_0"
        base, mt = _suffix_match(core)
        if mt is not None:
            # Re-attach the numeric tail to the base so files sharing the same
            # index group together: "wall_diff_0" and "wall_height_0" both
            # produce base "wall_0".
            return _TRIM_RE.sub("", base) + index_tail, mt
